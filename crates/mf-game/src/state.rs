...
//! only place that maps sim-transport readiness onto them.
...
pub struct PendingInit {
    ...
    /// message alongside city/difficulty so a re-init (menu Play) reuses the
    /// same world rather than silently re-rolling `rand_seed()` underneath the
    /// player.
    pub seed: u64,
...
/// Re-entering `MainMenu` (e.g. a fresh boot) must always start at `Title` —
/// never resume wherever the player happened to leave the menu screen state
/// last time.
...
/// Boot: load config, connect to the in-process embedded sim (infallible —
/// there is no socket or child process to fail), then move on to
/// `ConnectingSim`.
fn boot_system(
    ...
    // The all-Rust sim runs in-process; connecting never fails.
    commands.insert_resource(SimLink::connect_embedded());
...
fn on_exit_ingame_system(mut pause: ResMut<PauseState>) {
    pause.active = false;
}

/// ConnectingSim: send our hello, then wait for the sim's hello in
/// return (spec §1.4).
fn connecting_sim_system(
    ...
    // The link is inserted once at boot; reset the send-once latch when it
    // first appears so the handshake fires exactly once.
    ...
    for SimEvent(msg) in events.read() {
        if let mf_protocol::FromSimMsg::Json(FromSimJson::Hello(info)) = msg {
            if info.protocol_version != mf_protocol::PROTOCOL_VERSION {
                tracing::error!(
                    "mf-game: sim protocol version {} != client {}; aborting",
...
/// Graceful quit (spec §3.4): on `AppExit`, tell the in-process sim worker to
/// shut down so it stops cleanly.
