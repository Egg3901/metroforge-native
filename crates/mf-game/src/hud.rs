...
/// ConnectingSim must draw *something* (rather than a bare ClearColor) during
/// the brief in-process hello handshake, so the boot never looks frozen.
fn connecting_hud_system(mut contexts: EguiContexts) -> Result {
    let s = crate::strings::current();
    let ctx = contexts.ctx_mut()?;
