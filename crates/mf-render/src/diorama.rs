...
//! sides dropping ~80 m from the terrain edge, and those cut sides ARE the
...
/// The art direction says "~80 m"; terrain relief (re-based to min=0) can add
/// up to ~100 m on top so the walls read tall even on hilly cities.
pub const SLAB_DEPTH_M: f32 = 80.0;
...
const BEVEL_M: f32 = 5.0;
/// Half-thickness (m) of the water-table line drawn across the strata.
const WATER_BAND_HALF_M: f32 = 1.1;
...
fn build_diorama_system(
    ...
    for (normal, edge) in &edges {
        ...
        for w in cols.windows(2) {
            ...
            // quad; depths are metres below the surface, clamped to the slab
            // underside so the bedrock band always reaches the bottom.
            let y_at = |top: f32, depth: f64| -> f32 { (top - depth as f32).max(SLAB_BOTTOM_Y) };
            for bi in 0..4 {
                let ba = &col_a.bands[bi];
                let bb = &col_b.bands[bi];
                let color = palette::strata_color(ba.kind);
                let ya_top = y_at(wa_top, ba.top);
                let yb_top = y_at(wb_top, bb.top);
                let ya_bot = y_at(wa_top, ba.bottom);
                let yb_bot = y_at(wb_top, bb.bottom);
            ...
            let wta = y_at(wa_top, col_a.water_table_depth);
            let wtb = y_at(wb_top, col_b.water_table_depth);
    ...
    if medium_plus && !water.is_empty() {
        ...
        // Soft contact shadow: a large translucent dark quad just under the
        // slab so the model reads as floating over the backdrop void.
        let shadow_mesh = meshes.add(
            Plane3d::default()
                .mesh()
                .size(world_size * 1.35, world_size * 1.35)
                .build(),
        );
        let shadow_mat = materials.add(StandardMaterial {
            base_color: palette::contact_shadow(),
            unlit: true,
